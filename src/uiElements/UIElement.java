...
public class UIElement implements Interactable, UIAnimatable{
    ...
    public UIElement(int startX, int startY, int targetX, int targetY, String name, HashMap<UIElementState, Image> stateImages, Image[] animationImages, String[] tags) {
        ...
        this.collisionShape = new Rectangle(startX, startY, currentImage.getWidth(null), currentImage.getHeight(null));
        if (!(tags == null)) {
            this.tags = tags;
        } else {
...
//            if (animationStateNumber > (animationImages.length - 1) * animationFPSDivider){
//                originalImage = animationImages[animationImages.length-1];
//            } else {
//                originalImage = animationImages[animationStateNumber / animationFPSDivider];
//            }
            originalImage = currentImage;
        } else if (newState == UIElementState.HOVER){
            if (this.hasTag("NO_HOVER_IMAGE")){
        ...
        BufferedImage copiedImage = new BufferedImage(originalImage.getWidth(null),
...
//                System.out.println(e.getMessage());
    ...
    private void updateCollisions(UIController uiController){
        this.collisionShape.setLocation(getAbsoluteX(uiController), getAbsoluteY(uiController));
