...
//        System.out.println("AnimationStateNumber = " + animationStateNumber);
    ...
    public int move(boolean reversed, UIController uiController) {
        ...
        if (!(reversed)) {
            ...
            xDiff = startX - targetX;
            yDiff = startY - targetY;
        }
        if ((x == targetX && y == targetY) && (!(reversed)) || (x == startX && y == startY) && reversed){
            animationStateNumber = 0;
            return 0;
        }
...
//                System.out.println(e.getMessage());
    ...
    public void updateStateImage(UIElementState state, Image newImage, UIController uiController){
        if (newImage == null || newImage.getWidth(imageObserver) <= 0 || newImage.getHeight(imageObserver) <= 0){
            return;
        }
        UIElementState currentState = uiController.getElementState(name);
